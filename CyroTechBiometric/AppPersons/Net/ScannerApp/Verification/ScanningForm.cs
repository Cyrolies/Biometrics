...
namespace StudentScanner
{
	public partial class ScanningForm : Form
    {
        ...
        private void OnGetBaseTemplateComplete(bool bSuccess, int nRetCode)
        {
            ...
            if (bSuccess)
            {
                ...
                if (nResult == FutronicSdkBase.RETCODE_OK)
                {
                    ...
                    if (iRecords != -1)
                    {
                        int personID = Convert.ToInt32(Users[iRecords].UserName.Substring(Users[iRecords].UserName.IndexOf("-")+1));
                        Attendance att = new Attendance() {OrgID = Convert.ToInt32(ConfigurationManager.AppSettings["OrgID"]), CreateDateTime = DateTime.Now, PersonID = personID, Location=Environment.MachineName };
                        APIProxy proxy = new APIProxy();
                        string res = string.Empty;
		...
		private void timer1_Tick(object sender, EventArgs e)
		{
            ...
            {
                if (txtMessage.Text.Length > 0 && txtMessage.Text.Substring(0,3).ToUpper() != "PUT")
                {
                    Reset();
                    InitializerScanner();
                }
